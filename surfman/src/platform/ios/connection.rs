...
/// A no-op connection.
///
/// Connections to the CGS window server are implicit on iOS, so this is a zero-sized type.
#[derive(Clone)]
...
impl Connection {
    ...
    pub fn create_native_widget_from_rwh(
        ...
        use raw_window_handle::RawWindowHandle::IOS;

        match raw_handle {
            IOS(handle) => Ok(NativeWidget {
...
impl NativeConnection {
    ...
    ///
    /// This is a no-op on iOS, because Core Graphics window server connections are implicit in
    /// the platform APIs.
    #[inline]
